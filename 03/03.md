...
本分析針對顧客的三種狀態（Stayed, Churned, Joined）進行特徵比較，並深入探討已流失客群（Churned）的具體流失類別與原因。

## 1. 分析目標
* **(a) 全群組特徵分析：** 找出不同顧客狀態在數值型變數（如費用、年齡）與類別型變數（如合約、網路類型）上的差異。
* **(b) 流失客群深入分析：** 針對 `Churned` 群體，分析其流失的大類別佔比以及具體的離開原因排名。

## 2. Python 程式碼實作

以下程式碼使用 `pandas` 處理數據，並利用 `matplotlib` 與 `seaborn` 進行視覺化呈現。同時包含了解決中文字型顯示問題的設定。
