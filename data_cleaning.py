...
# (E) 檢查數值異常
if '每月費用' in df.columns:
    negative_count = (df['每月費用'] < 0).sum()
    if negative_count > 0:
        print(f" 發現 {negative_count} 筆每月費用為負值 (可能為折扣，保留)")
...
# (I) 流失相關欄位保留缺失值 (僅流失客戶有此資料)
if '客戶流失類別' in df.columns:
    null_count = df['客戶流失類別'].isnull().sum()
    print(f" '客戶流失類別': 保留 {null_count} 個缺失值 (僅流失客戶有此欄位)")

if '客戶離開原因' in df.columns:
    null_count = df['客戶離開原因'].isnull().sum()
    print(f" '客戶離開原因': 保留 {null_count} 個缺失值 (僅流失客戶有此欄位)")
